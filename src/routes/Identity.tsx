    ...
    <main className="mx-auto flex min-h-dvh w-full max-w-md flex-col gap-8 px-6 py-10">
      <header className="flex items-center justify-between gap-4">
        <div className="flex flex-col">
          ...
          <span className="truncate text-sm font-medium text-t1">{user?.email}</span>
        </div>
        <button
          type="button"
      ...
      <section className="rounded-2xl border border-b1 bg-bg-card p-6">
        ...
        <p className="mt-2 text-sm text-t2">
          Auth, routing, data client and the PWA shell are wired. The card, country switcher and
          profile surfaces land in the next wave, on top of the Supabase schema.
        </p>
      </section>
    </main>
  );
